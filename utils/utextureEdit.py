...
filename = os.path.join('input', f'{domname}_DOMAIN000.nc')
...
# for attr in texture.ncattrs():
#     print(f"{attr}: {texture.getncattr(attr)}")

# print("\nTexture values:")
# print(texture[:])

legend_text = texture.getncattr('legend')
legend_dict = {}
...
im = ax.imshow(texture_data, cmap='gray', origin='lower')
plt.colorbar(im, ax=ax)
ax.set_title('Texture Data')

# Create legend panel
ax_legend.axis('off')
ax_legend.set_title('Texture Legend', fontsize=12, fontweight='bold')
legend_lines = []
...
def format_coord(x, y):
    ...
    if 0 <= row < texture_data.shape[0] and 0 <= col < texture_data.shape[1]:
        val = int(texture_data[row, col])
...
def on_select(eclick, erelease):
    ...
    print("Texture types in selection:")
...
## Input fields for modifying texture
ax_texture = plt.axes([0.15, 0.12, 0.2, 0.05])
ax_percent = plt.axes([0.15, 0.05, 0.2, 0.05])
ax_apply = plt.axes([0.45, 0.05, 0.12, 0.05])
ax_save = plt.axes([0.6, 0.05, 0.12, 0.05])

textbox_texture = TextBox(ax_texture, 'New Texture: ', initial='')
...
def apply_changes(event):
    ...
    try:
        new_val = int(textbox_texture.text)
        percent = float(textbox_percent.text)
    except ValueError:
        print("Invalid input. Enter a number for texture and percent.")
        return

    if new_val not in legend_dict:
        print(f"Invalid texture value. Choose from: {list(legend_dict.keys())}")
        return
    ...
    for idx in points_to_change:
        ...
        texture_data[row, col] = new_val

    im.set_data(texture_data)
